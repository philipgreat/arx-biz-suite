...
import styles from './ViewDevice.table.less'
...
class ViewDeviceTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = ViewDeviceBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/viewDevice/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
