...
const internalImageListOf = (viewDevice) =>{
...
const internalSettingListOf = (viewDevice) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ViewDeviceService[candidateServiceName] 
...
const executeTrans = (viewDevice,targetComponent) =>{
...
const buildTransferModal = (viewDevice,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(viewDevice,targetComponent)}
...
const internalRenderExtraHeader = (viewDevice) =>{
	return null
}
const internalRenderExtraFooter = (viewDevice) =>{
	return null
}
...
class ViewDevicePreference extends Component {
...
}))(Form.create()(ViewDevicePreference))
