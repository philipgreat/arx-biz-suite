...
const showAssociateDialog = (targetComponent) => {
  ...
  <div>
	...
	data={{targetObjectList:selectedRows}} owner={owner}  
...
class TargetObjectSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {TargetObjectTable} = GlobalComponents;
    const {TargetObjectSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}${appLocaleName(userContext,"List")}`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TargetObjectSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,TargetObjectModalTable,"targetObjectIds")}
...
export default Form.create()(TargetObjectSearch)
