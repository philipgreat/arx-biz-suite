...
const internalImageListOf = (targetObject) =>{
...
const internalSettingListOf = (targetObject) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = TargetObjectService[candidateServiceName] 
...
const executeTrans = (targetObject,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_targetObject/doJob",payload: payload})
...
const buildTransferModal = (targetObject,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(targetObject,targetComponent)}
...
const internalRenderExtraHeader = (targetObject) =>{
	return null
}
const internalRenderExtraFooter = (targetObject) =>{
	return null
}
...
class TargetObjectPreference extends Component {
...
}))(Form.create()(TargetObjectPreference))
