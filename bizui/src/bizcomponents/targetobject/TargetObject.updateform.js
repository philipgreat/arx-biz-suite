...
import styles from './TargetObject.updateform.less'
...
class TargetObjectUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const targetObjectId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, targetObjectId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateTargetObject`,
          payload: {
            id: owner.id,
            type: 'targetObject',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextTargetObjectUpdateRow`,
        payload: {
          id: owner.id,
          type: 'targetObject',
          selectedRows,
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.height} {...formItemLayout}>
                  {getFieldDecorator('height', {
                    initialValue: selectedRow.height,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.textContent} {...formItemLayout}>
                  {getFieldDecorator('textContent', {
                    initialValue: selectedRow.textContent,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
...
}))(Form.create()(TargetObjectUpdateForm))
