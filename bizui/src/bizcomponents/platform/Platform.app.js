...
class PlatformBizApp extends React.PureComponent {
  ...
  getViewDeviceSearch = () => {
    ...
    return connect(state => ({
      ...
      name: "查看设备",
      role: "viewDevice",
      data: state._platform.viewDeviceList,
      metaInfo: state._platform.viewDeviceListMetaInfo,
      count: state._platform.viewDeviceCount,
      currentPage: state._platform.viewDeviceCurrentPageNumber,
      searchFormParameters: state._platform.viewDeviceSearchFormParameters,
      searchParameters: {...state._platform.searchParameters},
      expandForm: state._platform.expandForm,
      loading: state._platform.loading,
      partialList: state._platform.partialList,
      owner: { type: '_platform', id: state._platform.id, 
      referenceName: 'platform', 
      listName: 'viewDeviceList', ref:state._platform, 
      listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
    }))(ViewDeviceSearch)
  }
  getViewDeviceCreateForm = () => {
   	const {ViewDeviceCreateForm} = GlobalComponents;
   	const userContext = null
    return connect(state => ({
      rule: state.rule,
      role: "viewDevice",
      ...
      searchFormParameters: state._platform.viewDeviceSearchFormParameters,
      loading: state._platform.loading,
      owner: { type: '_platform', id: state._platform.id, referenceName: 'platform', listName: 'viewDeviceList', ref:state._platform, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
    }))(ViewDeviceCreateForm)
  }
  
  getViewDeviceUpdateForm = () => {
    const userContext = null
  	const {ViewDeviceUpdateForm} = GlobalComponents;
    return connect(state => ({
      selectedRows: state._platform.selectedRows,
      role: "viewDevice",
      currentUpdateIndex: state._platform.currentUpdateIndex,
      owner: { type: '_platform', id: state._platform.id, listName: 'viewDeviceList', ref:state._platform, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
